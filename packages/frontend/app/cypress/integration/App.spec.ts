describe('OPEN-SPACE e2e', function () {
  ...
  it('Submit a session', function () {
    cy.visit('/')
    cy.get('label').contains('Title').next('input').first().type('Serverless');
    cy.get('label').contains('Location').next('input').first().type('Location 3');
    cy.get('label').contains('Presenter').next('input').first().type('Enric');
    cy.get('label').contains('Time').next('input').first().type('15:00');
